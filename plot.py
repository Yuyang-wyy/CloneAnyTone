...
def analyze_pred_vs_actual(args):
    ...
    output_wav = os.path.join(model, f"y_test_full_{args.model_type}.wav")
    pred_wav = os.path.join(model, f"y_pred_full_{args.model_type}.wav")
    input_wav = os.path.join(model, f"x_test_full_{args.model_type}.wav")
    ...
    # Save the plot
    plt.savefig(os.path.join(model, "signal_comparison_e2s_" + str(round(e2s, 4)) + ".png"), bbox_inches="tight")
    ...
    plt.savefig(os.path.join(model, "detail_signal_comparison_e2s_" + str(round(e2s, 4)) + ".png"), bbox_inches="tight")
    ...
    plt.xlabel("Time [sec]")
    plt.savefig(os.path.join(model, "diff_spectrogram.png"), bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="models/pedalHybrid/Longermodel.ckpt")
    parser.add_argument("--model_type", type=str, default="hybrid_wavenet_unet",
                        choices=["transformer", "hybrid", "hybridplus", "hybrid_wavenet_unet"],
                        help="Model architecture type (used for result filenames)")
